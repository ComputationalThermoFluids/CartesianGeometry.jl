...
# 1D surface
function integrate!(moms, ::Type{Tuple{1}}, f, xyz::NTuple{1}; method=:vofi)
...
# 4D surface (volumes en 4D)
function integrate!(moms, ::Type{Tuple{1}}, f, xyz::NTuple{4}, bc)

    input = only.(axes.(xyz))
    linear = LinearIndices(input)

    x, y, z, w = xyz
    xex = zeros(Cdouble, 5) # 4 dimensions + 1 pour la mesure d'interface

    # x faces (volumes 3D fixés en x)
    ...
    for index in cartesian
        ...
        moms[4][n] = vofinit!(xex, f, SVector(x[i], x[i+1]),
                                     SVector(y[j], y[j+1]),
                                     SVector(z[k], z[k+1]),
                                     w[l])
    end
