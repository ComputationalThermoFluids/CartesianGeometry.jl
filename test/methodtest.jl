...
"""
Compare VOFI and SimpleVOF methods for volume and surface fractions
"""
function compare_methods_extended()
    println("Comparing VOFI vs SimpleVOF methods")
    println("===================================")
    
    # Generate grid with increasing resolution
    resolutions = [21, 41]
    
    # Test case: Circle centered at (0.5, 0.5) with radius 0.3
    center = (0.5, 0.5)
    radius = 0.3
    levelset = HyperSphere(radius, center)
    
    # Analytical values for comparison
    analytical_volume = π * radius^2
    analytical_perimeter = 2π * radius
    ...
    for resolution in resolutions
        ...
        for i in axes(bary_vofi, 1)
            ...
            if !isempty(valid_indices)
                ...
                bary_diff = [vofi - simple for (vofi, simple) in zip(vofi_values, simple_values)]
